...
TEST_CASE("PingManager concurrent execution", "[ping][concurrent]") {
    ...
    SECTION("Thread safety - multiple concurrent calls") {
        std::map<std::string, std::string> hosts1 = {
        ...
        std::vector<std::vector<std::tuple<std::string, std::string, bool, short, std::string>>> results1;
        std::vector<std::vector<std::tuple<std::string, std::string, bool, short, std::string>>> results2;
...
TEST_CASE("PingManager performance", "[ping][performance]") {
    ...
    SECTION("Large number of hosts") {
        std::map<std::string, std::string> hosts;
        for (int i = 1; i <= 10; ++i) {
            hosts[std::to_string(127) + "." + std::to_string(0) + "." + std::to_string(0) + "." + std::to_string(i)] = 
